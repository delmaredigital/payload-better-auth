...
- **`requireTwoFactor` option on `createBetterAuthPlugin` — deny document access until a second factor is enrolled.** Pass `true` (or an options object) to wrap every collection's and global's access control so authenticated users with `twoFactorEnabled !== true` are denied all operations (create/read/update/delete/readVersions/unlock). Deliberately not gated: anonymous requests (public reads keep working), the `admin` access key and Payload's internal collections (users must be able to reach the admin panel to enrol — pair with `admin.routes.unauthorized` pointing at your 2FA setup view), and machine credentials by default (`exemptMachineCredentials: true` — API-key/OAuth-bearer requests bypass the gate; machines can't enter a TOTP code and are already scope-constrained). Operations with undefined access are gated through Payload's default-equivalent ("any authenticated user"), so nothing slips through ungated. The gate is applied during `onInit`, against the fully-built sanitized config — after every plugin has finished shaping access control — so **plugin registration order doesn't matter**: an RBAC plugin registered after `createBetterAuthPlugin` is still wrapped. Warns at init when the Better Auth `twoFactor()` plugin isn't detected on the auth instance (the gate would otherwise lock out every user with no way to enrol). Options: `enabled` (env toggle), `fieldName`, `excludeCollections`/`excludeGlobals`, `exemptMachineCredentials`, and a custom `exempt(req)` escape hatch. The underlying `applyRequireTwoFactorGate(payload, options)` is exported for advanced/manual use.
