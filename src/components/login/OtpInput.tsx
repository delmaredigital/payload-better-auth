export function OtpInput({ id, value, onChange, length = 6, autoFocus = false }: { id: string; value: string; onChange: (v: string) => void; length?: number; autoFocus?: boolean }) {
  return (
    <input id={id} type="text" inputMode="numeric" autoComplete="one-time-code" value={value} required placeholder={'0'.repeat(length)} autoFocus={autoFocus}
